...
export class WordPressService {
   ...
   * 获取文章的 revisions
   */
  async getPostRevisions(id: number): Promise<WPPost[]> {
    return this.client.get(`/wp/v2/posts/${id}/revisions`)
      .then(res => res.data);
  }
   ...
   * 获取单个页面
   */
  async getPage(id: number | string): Promise<WPPage> {
    return this.client.get(`/wp/v2/pages/${id}`)
      .then(res => res.data);
  }
  ...
  async getMediaList(params?: {
    ...
    return this.client.get('/wp/v2/media', {
      params: {
        page: params?.page || 1,
        per_page: params?.perPage || 10,
        search: params?.search,
  ...
  async getUsers(params?: {
    ...
    return this.client.get('/wp/v2/users', {
      params: {
        page: params?.page || 1,
        per_page: params?.perPage || 10,
  ...
  async getUser(id: number): Promise<WPUser> {
    return this.client.get(`/wp/v2/users/${id}`)
      .then(res => res.data);
  }
